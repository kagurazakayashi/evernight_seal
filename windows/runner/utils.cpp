...
std::string Utf8FromUtf16(const wchar_t* utf16_string) {
  ...
  // 首先，使用安全的上限值找出字串長度（CWE-126）。
  // UNICODE_STRING_MAX_CHARS (32767) 是 UNICODE_STRING 的最大長度。
  int input_length = static_cast<int>(wcsnlen(utf16_string, UNICODE_STRING_MAX_CHARS));
  // 現在使用該有界長度來決定所需的緩衝區大小。
  // 當傳入明確長度時，WideCharToMultiByte 在其回傳大小中
  // 不會包含 null 終止符。
